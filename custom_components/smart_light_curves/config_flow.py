    ...
    async def async_step_user(self, user_input=None):
        ...
        data_schema = vol.Schema({
            ...
            vol.Required("occupancy_sensor"): selector.EntitySelector(
                selector.EntitySelectorConfig(domain="binary_sensor", device_class=["motion", "occupancy", "presence"]) 
            ),
...
class SmartLightCurvesOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        """Manage the options."""
        if user_input is not None:
            # Save the new settings into entry.options
            return self.async_create_entry(title="", data=user_input)

        # Helper to pre-fill the form with existing settings
        def get_cfg(key, default):
            return self.config_entry.options.get(key, self.config_entry.data.get(key, default))

        options_schema = vol.Schema({
            vol.Required("light_entity", default=get_cfg("light_entity", vol.UNDEFINED)): selector.EntitySelector(
                selector.EntitySelectorConfig(domain="light")
            ),
            vol.Required("lux_sensor", default=get_cfg("lux_sensor", vol.UNDEFINED)): selector.EntitySelector(
                selector.EntitySelectorConfig(domain="sensor", device_class="illuminance")
            ),
            vol.Required("occupancy_sensor", default=get_cfg("occupancy_sensor", vol.UNDEFINED)): selector.EntitySelector(
                selector.EntitySelectorConfig(domain="binary_sensor", device_class=["motion", "occupancy", "presence"]) 
            ),
            vol.Optional("kp", default=get_cfg("kp", 0.5)): selector.NumberSelector(
                selector.NumberSelectorConfig(min=0.0, max=10.0, step=0.1, mode=selector.NumberSelectorMode.BOX)
            ),
            vol.Optional("ki", default=get_cfg("ki", 0.01)): selector.NumberSelector(
                selector.NumberSelectorConfig(min=0.0, max=10.0, step=0.01, mode=selector.NumberSelectorMode.BOX)
            ),
            vol.Optional("kd", default=get_cfg("kd", 0.1)): selector.NumberSelector(
                selector.NumberSelectorConfig(min=0.0, max=10.0, step=0.1, mode=selector.NumberSelectorMode.BOX)
            ),
            vol.Optional("update_interval", default=get_cfg("update_interval", 5)): selector.NumberSelector(
                selector.NumberSelectorConfig(min=1, max=60, step=1, mode=selector.NumberSelectorMode.BOX)
            ),
        })
